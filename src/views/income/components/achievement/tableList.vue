    ...
    >
      <el-table-column v-if="timeType === 'month'" type="expand">
        <template #default="scope">
          <div class="px-10 py-2 expand-content">
            <div>
              <div class="mt-2">
                <Progress
                  :segments="
                    scope.row.allIncomeDetail.map(item => ({
                      percentage: divide(item.income, scope.row.income) * 100,
              ...
              :segments="[
                {
                  percentage: 100,
                  status: 'primary',
                  text: _.floor(scope.row.income).toLocaleString()
                }
              ]"
              height="25px"
            />
          ...
          <div>
            <Progress
              :segments="[
                {
                  percentage: _.floor(
                    divide(scope.row.income, scope.row.target) * 100
                  ),
                  status: 'primary',
                  text:
                    _.floor(divide(scope.row.income, scope.row.target) * 100) +
                    '%'
                }
              ]"
              height="25px"
      ...
      >
        <template #default="scope">
          <span
            ...
            :class="
              scope.row.yearOverYearGrowth < 0
                ? 'text-[var(--dash-red)]'
                : 'text-[var(--dash-green)]'
            "
          >
      ...
      <el-table-column prop="monthOverMonthGrowth" label="环比" width="80">
        <template #default="scope">
          <span
            class="text-center text-sm font-bold"
            :class="
              scope.row.monthOverMonthGrowth < 0
                ? 'text-[var(--dash-red)]'
                : 'text-[var(--dash-green)]'
            "
          >
