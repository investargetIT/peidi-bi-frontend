...
import { getCommonEnum, getGoodsReviewQuestion } from "@/api/taobao";
      ...
      if (res.code === 200) {
        ...
        function reworkAnalyzeNumCards() {
          ...
          const goodsAndCompareGoodsReviewsNum = {
            totalCompare: goodsReviewsNum.total - compareGoodsReviewsNum.total,
            positiveCompare:
          ...
          analyzeNumCardsTemp.push(
            {
              ...
              competitorContent: compareGoodsReviewsNum.total.toLocaleString(),
        ...
        function reworkCommentTrendsChart() {
          ...
          function calculateCommentTrends(dataList: AnalyzeListContentItem[]) {
        ...
        goodsWordCloud.value = goodsAndCompareGoodsWordCloudTemp.goodsWordCloud;
        ...
        function reworkAnalyzeLists() {
          ...
          analyzeListsTemp.push(
            {
              title: "本产品最新评论",
              contentList: reverseArrayByReviewDate(res.data.goodsReviews) || [] // 按照reviewDate排序
            },
            {
              title: "竞品最新评论",
              contentList:
    ...
    .then((res: any[]) => {
      if (res.every(item => item.code === 200)) {
        ...
        res[2].data.forEach((item: any) => {
          for (const ele of res[0].data) {
            productOptionsTemp.push({
              value: `${ele.value}&&${item.value}`,
              label: `${ele.value}-${item.value}`
            });
          }
          for (const ele of res[1].data) {
